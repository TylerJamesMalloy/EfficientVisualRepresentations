...
def aversion(participant_model, stimuli, idf, test=False):
    ...
    for _, trial in idf.iterrows(): 
        if(trial['Trial Type'] == 'Change Detection' and trial['Changed']):
            ...
            Losses.append(np.log(softmax[incorrect]))
            Accuracies.append(softmax[correct])
...
if __name__ == '__main__':
    ...
    modelChangeColumns = ["Id", "Model", "Split", "Log Likelihood", "Beta", "Upsilon"]
    modelChange = pd.DataFrame([], columns=modelChangeColumns)
    ...
    for idx, id in enumerate(ids):        
        ...
        #modelChangeColumns = ["Id", "Model", "Split", "Log Likelihood", "Coefficient"]
        d = pd.DataFrame([[id, "UBVAE", 100, best_nll, beta_coef, upsilon_coef]], columns=modelChangeColumns)
        modelChange = pd.concat([d, modelChange])
        ...
        upsilon_coef = coefs[np.argmin(results)][0]
        #modelChangeColumns = ["Id", "Model", "Split", "Log Likelihood", "Coefficient"]
        d = pd.DataFrame([[id, "UBVAE 80", 80, test_nll, beta_coef, upsilon_coef]], columns=modelChangeColumns)
        modelChange = pd.concat([d, modelChange])
        ...
        upsilon_coef = coefs[np.argmin(results)][0]
        #modelChangeColumns = ["Id", "Model", "Split", "Log Likelihood", "Coefficient"]
        d = pd.DataFrame([[id, "UBVAE 60", 60, test_nll, beta_coef, upsilon_coef]], columns=modelChangeColumns)
        modelChange = pd.concat([d, modelChange])
        ...
        upsilon_coef = coefs[np.argmin(results)][0]
        #modelChangeColumns = ["Id", "Model", "Split", "Log Likelihood", "Coefficient"]
        d = pd.DataFrame([[id, "UBVAE 40", 40, test_nll, beta_coef, upsilon_coef]], columns=modelChangeColumns)
        modelChange = pd.concat([d, modelChange])


    modelChange = modelChange.reset_index()
    modelChange.to_pickle("./modelChange.pkl")

    modelChange = pd.read_pickle("./modelChange.pkl")
    modelChange["Log Likelihood"] = -1 * modelChange["Log Likelihood"] 

    # ANOVA
    result = stats.f_oneway(modelChange['Log Likelihood'][modelChange['Model'] == "UBVAE"],
                            modelChange['Log Likelihood'][modelChange['Model'] == "UBVAE 80"],
                            modelChange['Log Likelihood'][modelChange['Model'] == "UBVAE 60"],
                            modelChange['Log Likelihood'][modelChange['Model'] == "UBVAE 40"])
    print(result)

    res = stats.tukey_hsd(modelChange['Log Likelihood'][modelChange['Model'] == "UBVAE"],
                            modelChange['Log Likelihood'][modelChange['Model'] == "UBVAE 80"],
                            modelChange['Log Likelihood'][modelChange['Model'] == "UBVAE 60"],
                            modelChange['Log Likelihood'][modelChange['Model'] == "UBVAE 40"])
    print(res)
    ...
    sns.barplot(modelChange, x="Model", y="Log Likelihood", hue="Split")
    plt.show()
