...
if(PLOTB):
       ...
       cols = ["Beta", "Outcome Variance Difference", "Probability of Selection"]
       df = pd.DataFrame([
              [4, 0.1, 0.4482],
              [4, 0.1, 0.4573],
              [4, 0.5, 0.7023],
       ...
       high = df[df["Beta"] == 100]
       low = df[df["Beta"] == 4]
       sns.regplot(high, x="Outcome Variance Difference", y="Probability of Selection", scatter=True, label="B=100", color="blue")
       sns.regplot(low, x="Outcome Variance Difference", y="Probability of Selection", scatter=True, label="B=4", color="orange")

       #sns.lineplot(df, x="Marble Utility", y="Utility Prediction Error", hue="Beta", palette=["Orange", "Blue"])

       plt.xlabel("Outcome Variance Difference " + r'$\sigma(x_1) - \sigma(x_2)$', fontsize = 14)
       plt.ylabel("Probability of Selection " + r'$p(x_1)$', fontsize = 14)
       plt.title("Probability of Selecting " +  r'$x_1$' + " by Outcome Variance Difference", fontsize = 16)
       plt.legend()
